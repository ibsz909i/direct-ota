...
## Unreleased candidate checks

On September 25, 2026, the exported Supabase provider was applied to a disposable local Supabase Docker stack with the real Storage service. The three current migrations applied. Anonymous and synthetic authenticated customer tokens could not upload or delete a test artifact; a service-created scoped upload capability uploaded once and refused overwrite; the public object returned the exact bytes and a `206` byte range. The test object and local containers/volumes were removed. Run `DIRECT_OTA_LOCAL_STORAGE_TEST=1 DIRECT_OTA_SUPABASE_WORKDIR=<disposable-workdir> node --test tests/supabase-storage.integration.test.mjs` to repeat that Storage check. The local Edge Function path remains covered by handler tests rather than a hosted gateway test; this local result does not prove hosted Supabase behavior.

The pending changes cover the coordinator, publisher tooling, diagnostics, scanner policy, GitHub Action, configurable artifact ceilings, native overlays, bounded aggregate metrics, optional stop-only rollout health gates, and signed background mode. The signed manifest remains protocol v1 with an optional mode field for new native runtimes. The local test suite includes deterministic telemetry sampling, throttled check hints, signed provenance tampering, local release inspection, custom scanner exceptions that cannot bypass built-in credential rules, and native drift diagnostics. A local `npm run check` passed with 72 tests passing and four environment-dependent tests skipped; the separate local Storage test passed. The Firebase Firestore/Storage emulator and isolated PostgreSQL tests passed separately. After applying the current native overlays to a disposable Capacitor 8 consumer, `doctor`, `cap sync`, iOS Simulator Xcode build, and Android Gradle debug build passed. These are build and local-service checks, not proof of OTA activation on physical devices. Repeat the full matrix after the final change set.

A disposable Capacitor 8 app installed an unreleased candidate tarball, synced iOS and Android, and passed `direct-ota doctor` with matching trust, runtime, and native copies. After the limit change, upgrading from the previous overlay initially exposed a blocked upgrade. Exact prior overlay hashes are now accepted; unknown modifications are still rejected. The upgraded app passed `doctor`, an iOS Simulator Xcode build, and an Android Gradle debug build. Swift verified a signed oversized manifest is accepted only when its larger bound is native-pinned. A Cloudflare D1 migration preservation test passed; the Cloudflare local Worker integration passed; an isolated PostgreSQL 17 test applied both Supabase migrations and verified the larger bucket/SQL ceiling. These builds and local migrations do not prove a device downloaded or activated an OTA. No hosted Supabase service or physical-device acceptance test was performed for this candidate.

The published Git tags `v0.1.0`, `v0.2.0`, `v0.3.0`, `v0.4.0`, and `v0.6.0` contain the same three native overlay hashes recorded in `native/overlay-history.json`. The native patcher therefore accepts those known prior overlays and still refuses unknown local modifications. Version 0.5.0 has no tag in this checkout; inspect an independently distributed 0.5.0 package before claiming its upgrade path. The expanded Swift corpus caught an invalid February 30 timestamp accepted by date normalization; the native verifier now round-trips the parsed UTC timestamp and requires its canonical string to match. Native UUID version, variant, and case validation now matches the shared protocol. The disposable iOS and Android native builds passed again after these changes.
