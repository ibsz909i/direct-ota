...
Set a request body limit slightly above the app's native-pinned `limits.archiveBytes` (5 MiB by default), finite timeouts, publishing admission limits, and connection ceilings at the proxy. Forward the `X-Direct-OTA-Upload` header on `/upload` and redact that header and request bodies from logs. The upload URL has no capability in its query string. Public check requests require no app login; `/publish` requires the configured ES256 publisher signature.
