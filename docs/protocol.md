...
Delta-capable native runtimes also accept optional signed `artifact.delta`. The immutable object then contains an encrypted complete ZIP followed immediately by an encrypted binary patch. `artifact.sha256` and `artifact.bytes` bind the whole object. `delta.fullBytes` and `delta.fullSha256` bind the first segment; `delta.offset`, `delta.bytes`, and `delta.sha256` bind the ranged patch. `delta.fromSha256` identifies the candidate used to build the patch; `delta.baseChecksum` must match a locally cached plaintext ZIP. Both segments have their own Capgo checksum and session-key envelope. The patch is used only with a matching cached base, and the reconstructed ZIP must match the full segment's signed plaintext checksum and pass the normal ZIP validation. Otherwise the client fetches the full segment. This optional field requires a delta-capable native runtime fingerprint. It never relaxes the full artifact limit.
...
For compound delta objects, the provider must return exact `206` ranges for both `[0, fullBytes)` and `[delta.offset, artifact.bytes)`. If a custom provider cannot serve these ranges, prepare a normal full release until it can. Native clients retain partial range downloads across restarts and verify the signed range digest before decryption. They retry a corrupt full partial once, then show a recoverable error without activating it.
