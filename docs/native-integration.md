...
2. In the host Capacitor 8 app, install the package tarball you actually downloaded, for example `npm install --save-exact ./direct-ota-0.7.0.tgz @capgo/capacitor-updater@8.51.25 @capacitor/app@8`. The 0.7.1 source candidate has no attached public tarball yet. Ensure Capacitor CLI 8 is installed in the host project. Both native plugins must be direct app dependencies. If `includePlugins` is set globally or per platform, include both there too. Add the iOS and Android platforms and run an initial `npx cap sync`.
