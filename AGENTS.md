...
- When a compatible earlier candidate is retained and the installed native build supports deltas, `prepare --delta-from PRIOR_RELEASE_DIR` can sign a smaller ranged patch and a complete fallback in one immutable object. Follow it with normal upload, internal promotion, remote doctor, and device verification. A delta never crosses native runtimes or substitutes for a first native store build.
