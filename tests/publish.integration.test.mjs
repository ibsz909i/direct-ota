...
test('CLI publishes, downloads, rolls out, rolls back and withdraws through HTTPS', {timeout:60000}, async t=>{
 ...
 const published=JSON.parse(await cli(['publish','--platform','ios','--version','1.0.2','--mode','background']));
 assert.equal(published.channel,'internal');assert.equal(published.sequence,2);
 const inspected=JSON.parse(await cli(['inspect','--release',published.release]));
 assert.equal(inspected.releaseId,published.releaseId);
 assert.equal(inspected.artifact.id,published.releaseId);
 assert.equal(inspected.provenanceVerified,true);
 assert.equal(inspected.mode,'background');
 assert.match(inspected.artifact.sha256,/^[0-9a-f]{64}$/);
 assert(inspected.artifact.bytes>5*1024*1024);
 const history=JSON.parse(await cli(['history','--platform','ios','--limit','1']));
 assert.equal(history.scope,'local');assert.equal(history.items.length,1);
 assert.equal(history.items[0].releaseId,published.releaseId);
 ...
 const remoteHistory=JSON.parse(await cli(['history','--remote','--platform','ios','--limit','1']));
 assert.equal(remoteHistory.scope,'remote');assert.equal(remoteHistory.items[0].releaseId,published.releaseId);
 assert.equal(remoteHistory.items[0].mode,'background');
 assert.equal(remoteHistory.nextCursor,2);
 const olderHistory=JSON.parse(await cli(['history','--remote','--platform','ios','--limit','1','--cursor','2']));
 assert.equal(olderHistory.items[0].sequence,1);assert.equal(olderHistory.nextCursor,null);
 const remoteInspect=JSON.parse(await cli(['inspect','--remote','--release-id',published.releaseId]));
 assert.equal(remoteInspect.artifact.sha256,inspected.artifact.sha256);
 ...
 assert.equal(JSON.parse(await cli(['rollout','--from',published.release,'--platform','ios','--channel','production','--rollout','1'])).sequence,1);
 assert.equal(JSON.parse(Buffer.from(JSON.parse(await cli(['status','--platform','ios','--channel','production'])).manifest.split('.')[1],'base64url')).mode,'background');
 assert.equal(JSON.parse(await cli(['rollout','--from',published.release,'--platform','ios','--channel','production','--rollout','100'])).sequence,2);
 assert.equal(JSON.parse(await cli(['rollback','--from',release,'--platform','ios','--channel','production'])).sequence,3);
 assert.equal(JSON.parse(Buffer.from(JSON.parse(await cli(['status','--platform','ios','--channel','production'])).manifest.split('.')[1],'base64url')).mode,undefined);
