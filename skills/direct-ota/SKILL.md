...
3. Run `npx direct-ota publish --platform ios|android --version SEMVER` for each target. It runs the host build, local native doctor, prepare, upload, internal promotion, and channel confirmation. Do not use raw service keys or edit production tables. If it fails after preparation, check status and use the retained release directory for a safe retry.
   For a delta-capable native build and a retained prior candidate on the same runtime, use `prepare --delta-from PRIOR_RELEASE_DIR` followed by `upload` and `promote` instead. The CLI keeps a complete fallback in the immutable artifact and omits the patch when it saves no bytes. Check the candidate's signed metadata and the target device before production rollout. Never reuse this option across native runtimes.
